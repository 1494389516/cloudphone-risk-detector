...
cprisk-armor 是编译后壳保护工具链，对 Mach-O 二进制执行 11 Pass 加固（字符串加密、Metadata 抹除、数据段加密、完整性锚点、结构混淆、符号表混淆、Pass 7 anti-debug 注入计划、Pass 8 ARM64 指令替换、Pass 9 ControlFlowOrchestrator、Pass 10 ImportEncryptor、Pass 11 HeaderEncryptor）。6.4 起 SDK 以静态库交付，壳对最终 App 二进制执行加固。6.5 起 Pass 4 写入白盒 PRF 四 section（`__swift5_awbm/awbc/awbd/awbt`），Pass 1/3/4 密钥通过白盒 PRF 派生，运行时优先走白盒路径。6.8 起 Pass 7 注入计划会被运行时真正消费为 inline patch gate；Pass 8 继续做 1:1 等长语义等价替换；Pass 9 结合 `cff_policy.yaml` 分配 CFF 计划并支持更多 dispatcher 风格；Pass 10 加密导入符号名，Pass 11 加密并备份 Mach-O header 关键字段。
