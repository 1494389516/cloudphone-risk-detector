...
| 加固项 | 原理 | 安全影响 |
|--------|------|----------|
| **TLS Certificate Pinning** | `CertificatePinningSessionDelegate` 基于 SPKI SHA-256 哈希固定证书，支持 iOS 15+ 和旧系统双路径 | 中间人无法篡改远程配置与策略 |
| **PLT Baseline 持久化** | `PLTIntegrityGuard` 首次运行基线存入 Keychain，后续校验对比持久化基线 | 防止 "hook 在先" 绕过（hook 在 SDK 加载前已安装） |
| **Keychain ACL 保护** | `KeychainDeviceID` 使用 `SecAccessControlCreateWithFlags` 替代 `kSecAttrAccessible` | Secure Enclave 设备上提高密钥导出难度 |

### 新增检测器（深度对抗）

| 检测器 | 检测维度 | 权重 | 原理 |
|--------|----------|------|------|
| `SensorReplayDetector` | 时间戳规律性 | 72 | systemUptime 采样 CV < 0.02 = 回放数据过于规整 |
| `SensorReplayDetector` | 探针延迟量化 | 72 | 随机 usleep 探针响应比例一致 = 缓冲区回放 |
| `SensorReplayDetector` | 噪声熵 | 72 | mach_absolute_time LSB 熵 < 2 bits = 虚拟/回放环境 |
| `GPURenderProbe` | Metal 设备特征 | 75 | 虚拟 GPU 名称 / 低 feature family / 小 maxBufferLength |
| `GPURenderProbe` | GPU 计算延迟 | 75 | buffer 分配 > 10ms / 空 command buffer > 5ms = 软件渲染 |
| `IsaSwizzleDetector` | isa swizzling | 82 | `object_getClass()` 校验单例类指针一致性 |
| `IsaSwizzleDetector` | 消息转发劫持 | 85 | 关键方法 IMP == `_objc_msgForward` = forwardInvocation 劫持 |
| `IsaSwizzleDetector` | 方法数量异常 | 50 | `class_copyMethodList` 计数超出预设范围 = 注入框架添加方法 |
...
| `sensor_replay_detected` | 72 | 传感器数据回放特征 |
| `gpu_render_anomaly` | 75 | GPU 渲染能力异常 |
| `isa_swizzle_detected` | 82 | isa 指针被篡改 |
| `msg_forward_hijack` | 85 | 消息转发劫持 |
| `method_count_anomaly` | 50 | 类方法数量异常 |
